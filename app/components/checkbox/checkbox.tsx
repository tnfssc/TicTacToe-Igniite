...
export function Checkbox(props: CheckboxProps) {
  ...
  const rootStyle = mergeAll(flatten([ROOT, props.style]).map(item => item || {}));
  const outlineStyle = mergeAll(flatten([OUTLINE, props.outlineStyle]).map(item => item || {}));
  const fillStyle = mergeAll(flatten([FILL, props.fillStyle]).map(item => item || {}));
